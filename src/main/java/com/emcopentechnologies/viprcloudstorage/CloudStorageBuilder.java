...
public class CloudStorageBuilder extends Builder {
	...
	public CloudStorageBuilder(String storageAccName, String containerName,
		...
		try {
			...
			if (filesDownloaded == 0) { // Mark build unstable if no files are
				...
				listener.getLogger().println(
						Messages.CloudStorageBuilder_nofiles_downloaded());
				build.setResult(Result.UNSTABLE);
			} else {
				listener.getLogger()
						.println(
								Messages.CloudStorageBuilder_files_downloaded_count(filesDownloaded));
			}
		} catch (Exception e) {
			e.printStackTrace(listener.error(Messages
					.CloudStorageBuilder_download_err(strAcc
							.getStorageAccName())));
			build.setResult(Result.UNSTABLE);
		}
		return true;
	...
	private boolean validateData(AbstractBuild build, BuildListener listener,
		...
		if (build.getResult() == Result.FAILURE) {
			listener.getLogger().println(
					Messages.CloudStorageBuilder_build_failed_err());
			return false;
		}
		...
		if (!Utils.validateBlobName(blobNamePrefix)) {
			listener.getLogger().println(
					Messages.CloudStorageBuilder_blobName_invalid());
			build.setResult(Result.UNSTABLE);
			return false;
		}
	...
	public CloudStorageBuilderDesc getDescriptor() {
		// see Descriptor javadoc for more about what a descriptor is.
		return (CloudStorageBuilderDesc) super.getDescriptor();
	}

	@Extension
	public static final class CloudStorageBuilderDesc extends
			Descriptor<Builder> {
		...
		public CloudStorageBuilderDesc() {
			super();
			load();
		}
		...
		/* public FormValidation doCheckIsDirectory(@QueryParameter String val) {
			...
			if (Utils.isNullOrEmpty(val) || !downloadDir.exists()) {
				...
				return FormValidation.error(Messages
						.CloudStorageBuilder_downloadDir_invalid());
			} else {
				return FormValidation.ok();
			}
		} */
		...
		public String getDisplayName() {
			return Messages.CloudStorageBuilder_displayName();
		}

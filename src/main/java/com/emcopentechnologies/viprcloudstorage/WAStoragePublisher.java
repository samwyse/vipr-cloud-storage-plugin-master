...
public class WAStoragePublisher extends Recorder {
	...
	/** ViPR Cloud storage container name. */
	private String containerName;

	/** ViPR Cloud storage container access. */
	private boolean cntPubAccess;

	/** ViPR Cloud storage container cleanup option. */
	...
	public Collection<? extends Action> getProjectActions(AbstractProject<?, ?> project) {
		CloudBlobProjectAction projectAction= new CloudBlobProjectAction(project);
	...
	public boolean perform(AbstractBuild<?, ?> build, Launcher launcher,
		...
		try {
			List<CloudBlob> blobs = new ArrayList<CloudBlob>();
			...
			if (filesUploaded == 0) { // Mark build unstable if no files are
				...
				if (blobs.size() > 0) {
	                build.getActions().add(new CloudBlobAction(build, strAcc.getStorageAccName(), expContainerName, blobs, allowAnonymousAccess));
				}
			}
		} catch (Exception e) {
	...
	public static final class WAStorageDescriptor extends
		...
		public FormValidation doCheckBlobName(@QueryParameter String val) {
			if (Utils.isNullOrEmpty(val)) {
				return FormValidation.error(Messages
						.CloudStorageBuilder_blobName_req());
			} else if (!Utils.validateBlobName(val)) {
				return FormValidation.error(Messages
						.CloudStorageBuilder_blobName_invalid());
			} else {
				return FormValidation.ok();
			}
